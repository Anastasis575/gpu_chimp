...
fn compress(v:f64,s:S,v_prev:f64,s_prev:S) -> Output{
    ...
    var xorred:u64= v_prev_u32^v_u32;

    var center_bits=u32(64-s.leading-s.trailing);

    //Output
    var content:vec2<u64>=vec2(0,0);
    var bit_count:u32=0;

    //case 1:  xor_value=0
    var case_1:vec2<u64>=vec2(0,0);
...
//    var leading_representation=u32(s.leading>=8&&s.leading<12)*1+u32(s.leading>=12&&s.leading<16)*2+u32(s.leading>=16&&s.leading<18)*3+u32(s.leading>=18&&s.leading<20)*4+u32(s.leading>=20&&s.leading<22)*5+u32(s.leading>=22&&s.leading<24)*6+u32(s.leading>=24)*7;
    ...
    var case_2:vec2<u64>=vec2(0u,1u);//code:01 bit_count=2
    ...
    var case_3:vec2<u64>=vec2(0,2); // code 10
    ...
    var case_4:vec2<u64>=vec2(0,3);// code 11
    ...
    var final_output=vec2(u64(final_output_i32.x),u64(final_output_i32.y));
...
fn pseudo_u64_shift(output:vec2<u64>,number:u32)->vec2<u32>{
    var first_number_bits:u32=extractBits(output.y,64-number,number);
...
fn pseudo_u64_add(output:vec2<u64>,number:u32)->vec2<u32>{
    // check if adding <number> causes an overflow
    var max_u64:u64=0xffffffffffffffffu;
    var isOverflow:u64=u64(output.y>=(max_u64-number));
    var isNotOverflow:u64=u64(abs(1-isOverflow));
    var diff:u64=max_u64-output.y;
