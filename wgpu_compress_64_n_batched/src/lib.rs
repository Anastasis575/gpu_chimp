...
mod tests {
    ...
    fn test_decompress_able() {
        ...
        for n in [32, 64, 128] {
            ...
            {
                println!("{file_name}");
                let filename = format!("{}_chimp64_n_{n}_output.txt", &file_name);
                if fs::exists(&filename).unwrap() {
                    fs::remove_file(&filename).unwrap();
                }
                let filename_no_io = format!("{}_chimp64_n_{n}_output_no_io.txt", &file_name);
                ...
                for size_checkpoint in 1..11 {
                        ...
                        .block_on()
                    {
                        Ok(decompressed_values) => {
                            ...
                            messages_no_io.push(DecompressionTime {
                                values: value_new.len() as u64,
