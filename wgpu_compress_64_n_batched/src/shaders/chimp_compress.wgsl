...
@group(0)
@binding(4)
var<storage, read_write> comprare_with: array<u32>; // this is used as both input and output for convenience



//#include(64_utils)

fn compress(v:f64,s:Ss,v_prev:f64,s_prev:Ss,c:u32) -> Output64{
    //@log2n
    //Conditions
    ...
    var center_bits=select (64,u64(64-s.leading-s.trailing),s.leading+s.trailing<=64);


    //case 1:  xor_value=0
    var case_1:vec2<u64>=vec2<u64>(0,u64(c));
    ...
    case_2.y+=extract_bits(u64(c),0u,log2n);
    case_2=pseudo_u64_shift(case_2,6u);
    case_2.y+=u64(extract_bits(u64(s.leading),0u,6u));
    case_2=pseudo_u64_shift(case_2,6u);
    case_2.y+=u64(extract_bits(center_bits,0u,6u));
    case_2=pseudo_u64_shift(case_2,u32(center_bits));
    case_2.y+=extract_bits(xorred,u32(s.trailing),u32(center_bits));
    ...
    case_3.y+=extract_bits(u64(c),0u,log2n);
    ...
    case_4.y+=extract_bits(u64(c),0u,log2n);
    case_4=pseudo_u64_shift(case_4,6u);
    case_4.y+=u64(extract_bits(u64(s.leading),0u,6u));
    ...
    var final_bit_count=log2n+
        s.equal*case_1_bit_count+ 
        (trail_gt_6*not_equal)*u32(case_2_bit_count)+
